...
**No free-item coupons.** A percentage or fixed discount is just arithmetic. A free item changes the basket, so the coupon would need to know product IDs and prices. That ties coupons to the product catalog for no real benefit.
...
It **never returns just true or false.** A rejection carries a reason: `Expired`, `MinimumSpendNotMet`, `NotFound`, `RedemptionLimitReached`. That one reason drives the message the customer sees, the log entry, and the test assertion from one place.
...
It **takes a subtotal, not a basket.** Every condition in the locked set needs only the order's value: expiry, minimum order value, redemption limit. Passing the full basket would hand the coupon project pizza IDs and prices, and a boundary that depends on the coupon code politely not looking is not a boundary. With `CouponBasket` it genuinely cannot see them. The cost is that any item-scoped coupon would require reopening this signature. "10% off pizzas only", buy one get one. That is a deliberate design change rather than a small edit.
...
The other options were worse. If ordering applied the coupon rules itself, the coupon logic would exist in two places. If the coupon service returned the final total, it would need to know pizza prices and delivery, which turns it into another ordering service.
...
In APIM, `validate-jwt` is scoped to the order endpoint rather than the whole API. That is a better demonstration of policy scoping than one blanket rule.
...
That makes SQL look up the identity in Entra ID, which means the SQL Server needs the **Directory Readers** role. That requires a tenant administrator, so it can't be done from an app pipeline. On someone else's tenant it may not be possible at all.
...
The result is a passwordless database with no tenant admin needed and no manual step. It adds about twenty lines in the pipeline and one Bicep property. That is a fair trade for a database that has no credential to leak, rotate, or store.
...
The storage account name is deterministic rather than random. It comes from `uniqueString(subscription().id, resourceGroup().name)`, and neither input changes when the group is deleted and recreated. So the frontend URL is **stable across a teardown and rebuild** and a registered redirect URI keeps working.

It is not knowable *before* the first deployment. A static-site hostname is `https://<account>.zNN.web.core.windows.net`, and the `zNN` segment is a DNS zone assigned when the account is created. It is `z29` for this deployment, and it is not predictable from the account name. So the redirect URI is registered after the first provision, and the pipeline fails the frontend stage if the deployed origin ever stops matching what was registered.
...
**Logged:** coupon results with their reason, order totals, redemption limits reached, and any coupon that passed preview but failed at submission. That last one is logged as a warning.
